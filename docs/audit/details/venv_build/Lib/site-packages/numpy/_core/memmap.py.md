# Documentación Técnica: `venv_build/Lib/site-packages/numpy/_core/memmap.py`

- **Ruta relativa:** `venv_build/Lib/site-packages/numpy/_core/memmap.py`
- **Tipo de archivo:** `.py`
...
#### Clase `memmap` (Línea 24)
- **Docstring:** _Create a memory-map to an array stored in a *binary* file on disk.
...
>>> fpo
memmap([  4.,   5.,   6.,   7.,   8.,   9.,  10.,  11.], dtype=float32)_
- **Métodos:**
  - `def __new__(cls, filename, dtype, mode, offset, shape, order)` (Línea 215): Sin docstring.
  - `def __array_finalize__(self, obj)` (Línea 311): Sin docstring.
  - `def flush(self)` (Línea 323): Write any changes in the array to the file on disk.
...
--------
memmap
  - `def __array_wrap__(self, arr, context, return_scalar)` (Línea 341): Sin docstring.
  - `def __getitem__(self, index)` (Línea 358): Sin docstring.
