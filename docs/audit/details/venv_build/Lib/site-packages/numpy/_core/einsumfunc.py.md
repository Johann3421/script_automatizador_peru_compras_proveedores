# Documentación Técnica: `venv_build/Lib/site-packages/numpy/_core/einsumfunc.py`

- **Ruta relativa:** `venv_build/Lib/site-packages/numpy/_core/einsumfunc.py`
- **Tipo de archivo:** `.py`
- **Líneas de código:** 1647
...
#### `def _flop_count(idx_contraction, inner, num_terms, size_dictionary)` (Línea 23)
- **Propósito:** Computes the number of FLOPS in the contraction.
...
60
- **Firma:** `def _flop_count(idx_contraction, inner, num_terms, size_dictionary)`
- **Retorno / Efectos:** Consulta código fuente.

#### `def _compute_size_by_dict(indices, idx_dict)` (Línea 61)
- **Propósito:** Computes the product of the elements in indices based on the dictionary
idx_dict.
...
90
- **Firma:** `def _compute_size_by_dict(indices, idx_dict)`
- **Retorno / Efectos:** Consulta código fuente.

#### `def _find_contraction(positions, input_sets, output_set)` (Línea 90)
- **Propósito:** Finds the contraction for a given set of input and output sets.
...
- **Firma:** `def _find_contraction(positions, input_sets, output_set)`
- **Retorno / Efectos:** Consulta código fuente.

#### `def _optimal_path(input_sets, output_set, idx_dict, memory_limit)` (Línea 150)
- **Propósito:** Computes all possible pair contractions, sieves the results based
...
[(0, 2), (0, 1)]
- **Firma:** `def _optimal_path(input_sets, output_set, idx_dict, memory_limit)`
- **Retorno / Efectos:** Consulta código fuente.

#### `def _parse_possible_contraction(positions, input_sets, output_set, idx_dict, memory_limit, path_cost, naive_cost)` (Línea 224)
- **Propósito:** Compute the cost (removed size + flops) and resultant indices for
...
new_input_sets : list of sets
    ...
    is performed.
- **Firma:** `def _parse_possible_contraction(positions, input_sets, output_set, idx_dict, memory_limit, path_cost, naive_cost)`
- **Retorno / Efectos:** Consulta código fuente.

#### `def _update_other_results(results, best)` (Línea 288)
- **Propósito:** Update the positions and provisional input_sets of ``results``
...
mod_results : list
    ...
    ``best`` contraction.
- **Firma:** `def _update_other_results(results, best)`
- **Retorno / Efectos:** Consulta código fuente.

#### `def _greedy_path(input_sets, output_set, idx_dict, memory_limit)` (Línea 330)
- **Propósito:** Finds the path by contracting the best pair until the input list is
...
[(0, 2), (0, 1)]
- **Firma:** `def _greedy_path(input_sets, output_set, idx_dict, memory_limit)`
- **Retorno / Efectos:** Consulta código fuente.

#### `def _parse_einsum_input(operands)` (Línea 445)
- **Propósito:** A reproduction of einsum c side einsum parsing in python.
...
('za,xza', 'xz', [a, b]) # may vary
- **Firma:** `def _parse_einsum_input(operands)`
- **Retorno / Efectos:** Consulta código fuente.

#### `def _einsum_path_dispatcher()` (Línea 624)
- **Propósito:** Sin docstring.
- **Firma:** `def _einsum_path_dispatcher()`
- **Retorno / Efectos:** Consulta código fuente.

#### `def einsum_path()` (Línea 635)
- **Propósito:** einsum_path(subscripts, *operands, optimize='greedy')
...
- **Firma:** `def einsum_path()`
- **Retorno / Efectos:** Consulta código fuente.

#### `def _parse_eq_to_pure_multiplication(a_term, shape_a, b_term, shape_b, out)` (Línea 923)
- **Propósito:** If there are no contracted indices, then we can directly transpose and
...
- **Firma:** `def _parse_eq_to_pure_multiplication(a_term, shape_a, b_term, shape_b, out)`
- **Retorno / Efectos:** Consulta código fuente.

#### `def _parse_eq_to_batch_matmul(eq, shape_a, shape_b)` (Línea 969)
- **Propósito:** Cached parsing of a two term einsum equation into the necessary
...
- **Firma:** `def _parse_eq_to_batch_matmul(eq, shape_a, shape_b)`
- **Retorno / Efectos:** Consulta código fuente.

#### `def _parse_output_order(order, a_is_fcontig, b_is_fcontig)` (Línea 1126)
- **Propósito:** Sin docstring.
- **Firma:** `def _parse_output_order(order, a_is_fcontig, b_is_fcontig)`
- **Retorno / Efectos:** Consulta código fuente.

#### `def bmm_einsum(eq, a, b, out)` (Línea 1144)
- **Propósito:** Perform arbitrary pairwise einsums using only ``matmul``, or
...
- **Firma:** `def bmm_einsum(eq, a, b, out)`
- **Retorno / Efectos:** Consulta código fuente.

#### `def _einsum_dispatcher()` (Línea 1234)
- **Propósito:** Sin docstring.
- **Firma:** `def _einsum_dispatcher()`
- **Retorno / Efectos:** Consulta código fuente.

#### `def einsum()` (Línea 1243)
- **Propósito:** einsum(subscripts, *operands, out=None, dtype=None, order='K',
