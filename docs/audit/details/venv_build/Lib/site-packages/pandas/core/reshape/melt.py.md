# Documentación Técnica: `venv_build/Lib/site-packages/pandas/core/reshape/melt.py`

- **Ruta relativa:** `venv_build/Lib/site-packages/pandas/core/reshape/melt.py`
- **Tipo de archivo:** `.py`
- **Líneas de código:** 676
...
#### `def ensure_list_vars(arg_vars, variable, columns)` (Línea 30)
- **Propósito:** Sin docstring.
- **Firma:** `def ensure_list_vars(arg_vars, variable, columns)`
- **Retorno / Efectos:** Consulta código fuente.

#### `def melt(frame, id_vars, value_vars, var_name, value_name, col_level, ignore_index)` (Línea 45)
- **Propósito:** Unpivot a DataFrame from wide to long format, optionally leaving identifiers set.
...
1      b          B          E      3
2      c          B          E      5
- **Firma:** `def melt(frame, id_vars, value_vars, var_name, value_name, col_level, ignore_index)`
- **Retorno / Efectos:** Consulta código fuente.

#### `def lreshape(data, groups, dropna)` (Línea 282)
- **Propósito:** Reshape wide-format data to long. Generalized inverse of DataFrame.pivot.
...
2  Red Sox  2008  545
3  Yankees  2008  526
- **Firma:** `def lreshape(data, groups, dropna)`
- **Retorno / Efectos:** Consulta código fuente.

#### `def wide_to_long(df, stubnames, i, j, sep, suffix)` (Línea 369)
- **Propósito:** Unpivot a DataFrame from wide to long format.
...
3     1     one  2.2
      ...
      3     one  2.1
            two  2.9
- **Firma:** `def wide_to_long(df, stubnames, i, j, sep, suffix)`
- **Retorno / Efectos:** Consulta código fuente.
