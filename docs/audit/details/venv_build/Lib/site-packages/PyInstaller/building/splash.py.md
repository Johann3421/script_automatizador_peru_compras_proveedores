# Documentación Técnica: `venv_build/Lib/site-packages/PyInstaller/building/splash.py`

- **Ruta relativa:** `venv_build/Lib/site-packages/PyInstaller/building/splash.py`
- **Tipo de archivo:** `.py`
- **Líneas de código:** 513
...
#### Clase `Splash` (Línea 28)
- **Docstring:** _Bundles the required resources for the splash screen into a file, which will be included in the CArchive.

A Splash has two outputs, one is itself and one is stored in splash.binaries. Both need to be passed to other
build targets in order to enable the splash screen._
- **Métodos:**
  - `def __init__(self, image_file, binaries, datas)` (Línea 35): :param str image_file:
...
:type center: str
  - `def _check_guts(self, data, last_build)` (Línea 318): Sin docstring.
  - `def assemble(self)` (Línea 329): Sin docstring.
  - `def _check_tcl_tk_compatibility(tcltk_info)` (Línea 427): Sin docstring.
  - `def generate_script(self)` (Línea 471): Generate the script for the splash screen.

If minify_script is True, all unnecessary parts will be removed.
  - `def _uses_tkinter(tkinter_file, binaries)` (Línea 506): Sin docstring.
